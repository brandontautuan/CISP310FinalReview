...
\title{Finalexampractice Debugged}
\author{}
\date{}
...
\section{Final Exam Practice - Debugged and Fully Commented}
...
\begin{enumerate}
\item \textbf{Understand the C Code}: Start by reading the C code to understand what the program should do
\item \textbf{Trace the Stack}: Draw the stack frame layout for each function, tracking where parameters and locals are
\item \textbf{Follow Register Values}: For each instruction, track what value each register contains
\item \textbf{Verify Parameter Access}: Check that parameters are accessed using correct offsets from stack pointer
\item \textbf{Check Stack Balance}: Ensure pushes match pops, and caller/callee responsibilities are met
\item \textbf{Verify Control Flow}: Trace through if/while loops to ensure logic matches C code
\item \textbf{Check Type Sizes}: Verify offsets account for correct sizes (1 byte for pointers, not struct sizes)
\item \textbf{Review Register Usage}: Ensure registers aren't overwritten before their values are used
\end{enumerate}

\subsection{Key Principles Applied}

\begin{itemize}
\item \textbf{Stack grows downward}: Pushing decrements D, popping increments D
\item \textbf{Parameters are at positive offsets} from where D points at function entry
\item \textbf{Local variables are at base} (D+0) after allocation
\item \textbf{Arguments pushed in reverse order}: Last argument first, first argument last
\item \textbf{Caller cleans arguments, callee cleans return address}
\item \textbf{Struct members have fixed offsets} from struct base address
\item \textbf{Pointers are 1 byte} in TTPASM, not the size of what they point to
\end{itemize}

\hrule

\subsection{Bug Analysis and Fixes}
...
\subsection{BUG 1: Line 60 - Missing Stack Pointer Initialization}
\textbf{Issue}: \texttt{ldi d,} has no value, cannot initialize stack pointer
\textbf{Fix}: \texttt{ldi d, 0} to initialize stack pointer to 0

\subsection{BUG 2: Line 107 - Wrong Offset Calculation for result}
\textbf{Issue}: \texttt{isLoop\textit{result: isLoop}fPtr Node\textit{size +} uses \texttt{Node}size} (2 bytes) when \texttt{fPtr} is a pointer (1 byte)
\textbf{Fix}: \texttt{isLoop\textit{result: isLoop}fPtr 1 +} since pointers are 1 byte in TTPASM

\subsection{BUG 3: Line 112 - Incorrect Local Variable Allocation Syntax}
\textbf{Issue}: \texttt{ldi c,0 isLoop_lvs -} is invalid syntax - cannot subtract in ldi
\textbf{Fix}: \texttt{ldi c, isLoop_lvs} then \texttt{sub d, c} to allocate locals

\subsection{BUG 4: Lines 119-121 - Incorrect Parameter Access}
\textbf{Issue}: Loading \texttt{pNode} incorrectly - \texttt{ld a,(a)} tries to dereference the offset, not the parameter
\textbf{Fix}: Must compute address of parameter first: \texttt{ldi a, isLoop_pNode} then \texttt{add a, d} then \texttt{ld a, (a)}

\subsection{BUG 5: Lines 124-129 - Incorrect prevFrame Assignment to f.prevFrame}
\textbf{Issue}: Complex incorrect calculation with \texttt{Frame\textit{prevFrame Frame}pNode -} which is wrong
\textbf{Fix}: Simply load prevFrame parameter and store to f.prevFrame

\subsection{BUG 6: Lines 136-137 - Wrong Way to Access Parameter prevFrame}
\textbf{Issue}: Using \texttt{Frame\textit{prevFrame} (struct member offset) instead of parameter offset \texttt{isLoop}prevFrame}
\textbf{Fix}: Use \texttt{ldi a, isLoop_prevFrame} then \texttt{add a, d} then \texttt{ld a, (a)}

\subsection{BUG 7: Line 169 - Wrong Register Used to Store fPtr}
\textbf{Issue}: \texttt{st (c),b} - register \texttt{c} doesn't contain the address of fPtr here
\textbf{Fix}: Must recompute address: \texttt{ldi c, isLoop_fPtr} then \texttt{add c, d} then \texttt{st (c), b}

\subsection{BUG 8: Line 172 - Register c May Not Contain fPtr}
\textbf{Issue}: \texttt{and c,c} tests register c, but c was last used for saving/restoring and may not contain fPtr
\textbf{Fix}: Must reload fPtr: \texttt{ldi c, isLoop_fPtr} then \texttt{add c, d} then \texttt{ld c, (c)} then test

\subsection{BUG 9: Line 197 - Wrong Register Contains Result}
\textbf{Issue}: \texttt{st (b),c} - register c may not contain the OR result
\textbf{Fix}: After OR logic, result should be in a specific register, then store it

\subsection{BUG 10: Line 182 - Wrong Offset for Parameter}
\textbf{Issue}: \texttt{ldi b,isLoop_pNode 1 +} - adding 1 doesn't skip return address properly
\textbf{Fix}: Should use \texttt{isLoop_pNode} directly since it already accounts for local vars + ret addr

\subsection{BUG 11: Line 218 - Missing Value for Second Argument}
\textbf{Issue}: \texttt{ldi a,} has no value for pushing prevFrame argument (should be 0)
\textbf{Fix}: \texttt{ldi a, 0}

\subsection{BUG 12: Line 232 - Missing Value for Argument Cleanup}
\textbf{Issue}: \texttt{ldi b,} has no value for cleaning up 2 arguments
\textbf{Fix}: \texttt{ldi b, 2} then \texttt{add d, b} or use two \texttt{inc d} instructions

\subsection{BUG 13: Line 199 - Wrong Label Name}
\textbf{Issue}: Label is \texttt{isLoop\textit{endif0} but jump at line 132 goes to \texttt{isLoop}endif}
\textbf{Fix}: Should be consistent - use \texttt{isLoop_endif}

\hrule

\subsection{FULLY DEBUGGED CODE WITH COMMENTS}
...
\begin{enumerate}
\item \textbf{Line 60}: Added missing value \texttt{0} to \texttt{ldi d, 0} for stack initialization
\item \textbf{Line 107}: Changed \texttt{Node_size +} to \texttt{1 +} since fPtr is 1 byte, not 2
\item \textbf{Line 112}: Fixed allocation syntax from \texttt{ldi c,0 isLoop\textit{lvs -} to \texttt{ldi c, isLoop}lvs} then \texttt{sub d, c}
\item \textbf{Lines 119-121}: Fixed parameter access by properly computing address with \texttt{add a, d} before \texttt{ld a, (a)}
\item \textbf{Lines 124-129}: Fixed prevFrame assignment by correctly loading parameter and storing to struct member
\item \textbf{Lines 136-137}: Fixed parameter access by using \texttt{isLoop\textit{prevFrame} instead of \texttt{Frame}prevFrame}
\item \textbf{Line 169}: Fixed fPtr update by recomputing address of fPtr variable
\item \textbf{Line 172}: Fixed fPtr test by reloading fPtr value before testing
\item \textbf{Line 182}: Fixed offset calculation for pNode parameter access in recursive call
\item \textbf{Line 197}: Result storage is now correct after fixing register tracking
\item \textbf{Line 218}: Added missing value \texttt{0} for prevFrame argument
\item \textbf{Line 232}: Fixed argument cleanup by using two \texttt{inc d} instructions
\item \textbf{Line 199}: Fixed label name from \texttt{isLoop\textit{endif0} to \texttt{isLoop}endif}
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Stack Frame Management}: Proper allocation/deallocation of local variables
\item \textbf{Parameter Access}: Computing addresses using offsets from stack pointer
\item \textbf{Struct Member Access}: Using struct member offsets to access nested data
\item \textbf{Pointer Dereferencing}: Loading addresses, then dereferencing to get values
\item \textbf{Recursive Function Calls}: Proper argument passing and cleanup
\item \textbf{Register Tracking}: Maintaining awareness of what each register contains
\item \textbf{Stack Balance}: Ensuring stack is balanced after function calls
\item \textbf{Control Flow}: Implementing while loops and conditional logic
\item \textbf{OR Short-Circuit Evaluation}: Implementing \texttt{A || B} with short-circuit
\item \textbf{Array Indexing}: Computing addresses for array elements
\end{enumerate}
